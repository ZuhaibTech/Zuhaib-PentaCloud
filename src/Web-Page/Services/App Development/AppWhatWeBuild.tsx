        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {apps.map((app, i) => (
            <motion.div
              ...
              className="bg-[#E8F0F8] p-8 rounded-[40px] shadow-[10px_10px_20px_rgba(163,185,210,0.6),-10px_-10px_20px_rgba(255,255,255,0.95)] hover:shadow-[20px_20px_40px_rgba(163,185,210,0.7),-20px_-20px_40px_rgba(255,255,255,1)] group transition-all duration-500"
            >
              <div 
                className="w-16 h-16 rounded-[24px] bg-[#E8F0F8] shadow-[inset_4px_4px_8px_rgba(163,185,210,0.4),inset_-4px_-4px_8px_rgba(255,255,255,0.8)] group-hover:shadow-[4px_4px_12px_rgba(163,185,210,0.5),-4px_-4px_12px_rgba(255,255,255,0.9)] flex items-center justify-center mb-8 group-hover:rotate-[15deg] transition-all duration-500"
                style={{ color: app.color }}
              >
                <app.icon size={30} />
              </div>

              <h3 className="font-nunito font-black text-[22px] text-[#0D1B2A] mb-4 group-hover:text-[#1A7FD4] transition-colors">{app.title}</h3>
              <p className="font-inter text-[14px] leading-relaxed text-[#4A6080] mb-8">
                {app.desc}
              </p>

              <div className="flex flex-wrap gap-2 mb-8 h-16">
                {app.tags.map((tag, j) => (
                  <span key={j} className="bg-[#E8F0F8] shadow-[2px_2px_4px_rgba(163,185,210,0.3),-2px_-2px_4px_rgba(255,255,255,0.8)] text-[#1A7FD4] font-nunito font-bold text-[9px] px-3 py-1 rounded-full uppercase tracking-wider group-hover:scale-105 transition-transform">
                    {tag}
                  </span>
                ))}
              </div>
              ...
              {/* Mini Visuals */}
              <div className="pt-6 border-t border-[#1A7FD4]/08">
                {app.visual === "ios" && (
                  <div className="flex items-center justify-between">
                    <div className="flex flex-col gap-1">
                       <div className="text-[10px] font-bold text-[#4A6080] uppercase tracking-tighter">App Rating</div>
                       <div className="flex gap-0.5 text-amber-500">
                          {[...Array(5)].map((_, j) => <Star key={j} size={10} fill="currentColor" />)}
                       </div>
                    </div>
                    <div className="text-[20px] font-black text-[#0D1B2A] flex items-baseline">
                      4.9<span className="text-[10px] text-[#4A6080] font-bold ml-1">/5</span>
                    </div>
                  </div>
                )}
                {app.visual === "enterprise" && (
                   <div className="w-full flex flex-col gap-2">
                      ...
                      <div className="flex justify-between text-[9px] font-black text-[#4A6080] uppercase">
                         <span>CPU usage</span>
                ...
                {app.visual === "fintech" && (
                   <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-full bg-[#E8F0F8] shadow-[inset_2px_2px_4px_rgba(163,185,210,0.3)] flex items-center justify-center text-[#34C98A]">
                         <ShieldCheck size={20} />
                      </div>
                      <div className="flex-1">
                         <div className="h-1 w-full bg-[#E8F0F8] shadow-inner rounded-full mb-1" />
                         <div className="text-[8px] font-black text-[#34C98A] uppercase tracking-widest">Biometric Encrypted</div>
                      </div>
                   </div>
                )}
                {app.visual === "social" && (
                   <div className="flex items-center gap-2">
                      <div className="flex -space-x-2">
                         {[
                           ...
                           <div key={j} className="w-8 h-8 rounded-full border-2 border-[#E8F0F8] bg-slate-200 shadow-md overflow-hidden">
                              <img src={src} className="w-full h-full object-cover" alt={`User ${j}`} />
                           </div>
                         ))}
                      </div>
                      <div className="text-[9px] font-black text-[#F59E0B] uppercase">
                        +1.2k Active Now
                      </div>
                   </div>
                )}
